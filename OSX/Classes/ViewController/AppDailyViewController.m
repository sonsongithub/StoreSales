...
@implementation AppDailyViewController
...
- (void)reload {
	...
	if (UIAppDelegate.currentOrderType == CellOrderUnits) {
		sql = "select beginDate, endDate, sum(units) from daily, currencyTableUSD where productTypeIdentifier != 7 and currencyTableUSD.code = royaltyCurrency and appleIdentifier = ? group by beginDate order by endDate desc";
	}
	else if (UIAppDelegate.currentOrderType == CellOrderSales) {
		sql = "select beginDate, endDate, sum(units * royaltyPrice/currencyTableUSD.rate*?) from daily, currencyTableUSD where royaltyPrice > 0 and productTypeIdentifier !=7 and currencyTableUSD.code = royaltyCurrency and appleIdentifier = ? group by beginDate order by endDate desc";
	}
	...
	if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) != SQLITE_OK) {
		...
		if (UIAppDelegate.currentOrderType == CellOrderUnits) {
			sqlite3_bind_int(statement, 1, self.currentSales.info.appleIdentifier);
		}
		else if (UIAppDelegate.currentOrderType == CellOrderSales) {
			sqlite3_bind_double(statement, 1, UIAppDelegate.userCurrencyRate);
			sqlite3_bind_int(statement, 2, self.currentSales.info.appleIdentifier);
		}
		else if (UIAppDelegate.currentOrderType == CellOrderUpgrade) {
			sqlite3_bind_int(statement, 1, self.currentSales.info.appleIdentifier);
		}
...
- (void)viewWillAppear:(BOOL)animated {
	...
	[self reload];
	self.navigationItem.title = NSLocalizedString(@"Daily Reports", nil);
