...
- (void)updateTitle {
	...
	NSDate *endDate = nil;
	char *sql = "select min(beginDate), max(endDate) from weekly";
...
- (NSString*)titleForLineChart {
	...
	if (UIAppDelegate.currentOrderType == CellOrderSales) {
		lineChartTitle = [NSString stringWithFormat:NSLocalizedString(@"Weekly - Sales (%@ - %@)", nil), [temp stringFromDate:beginDate], [temp stringFromDate:endDate]];
	}
	else if (UIAppDelegate.currentOrderType == CellOrderUnits) {
		lineChartTitle = [NSString stringWithFormat:NSLocalizedString(@"Weekly - Units (%@ - %@)", nil), [temp stringFromDate:beginDate], [temp stringFromDate:endDate]];
	}
	else if (UIAppDelegate.currentOrderType == CellOrderUpgrade) {
		lineChartTitle = [NSString stringWithFormat:NSLocalizedString(@"Weekly - Upgrade (%@ - %@)", nil), [temp stringFromDate:beginDate], [temp stringFromDate:endDate]];
	}
	[temp release];
...
- (GraphView*)graphView {
	WeeklyLineChartView *view = [[WeeklyLineChartView alloc] initWithFrame:CGRectMake(0, 0, 480, 300)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	WeeklyCell *cell = (WeeklyCell*)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
	if (cell == nil) {
		cell = [[[WeeklyCell alloc] initWithStyle:UITableViewStylePlain reuseIdentifier:CellIdentifier] autorelease];
	}
