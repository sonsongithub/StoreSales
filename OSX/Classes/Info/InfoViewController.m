...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
	...
	if (actionSheet.tag == 0) {
		if (buttonIndex == 0) {
			...
			//
			[picker setSubject:NSLocalizedString(@"[StoreSales contact] ", nil)];
		...
		if (buttonIndex == 1) {
			...
			//
			[[SQLiteDBController sharedInstance] deleteAllRecordFromAllTables];
			
			// Delete cache files
			[UIAppDelegate deleteAllCachePlist];
			
			UIAlertView *alertView = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil)
																message:NSLocalizedString(@"You have to pairing this iPhone with your mac", nil)
															   delegate:nil
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
	...
	if (section == 1) {
		return NSLocalizedString(@"Initialize", nil);
	}
	return nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) {
		...
		if (indexPath.row == 4) {
			cell.textLabel.text = NSLocalizedString(@"Disclaimer", nil);
			cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
		}
		if (indexPath.row == 5) {
		...
		if (indexPath.row == 0) {
			cell.textLabel.text = NSLocalizedString(@"Delete all data", nil);
			cell.accessoryType = UITableViewCellAccessoryNone;
		}
	}
    return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) {
		...
		if (indexPath.row == 4) {
			DisclaimerViewController *controller = [DisclaimerViewController defaultController];
			[self.navigationController pushViewController:controller animated:YES];
		}
		if (indexPath.row == 5) {
			// Clicked "Contact"
